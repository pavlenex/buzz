...
Clarify the goal, identify the audience, and call out the tradeoffs that matter. Keep recommendations concrete and avoid over-polishing before the shape of the work is understood.

# Collaboration

Ask sharp questions when the brief is ambiguous. Once the direction is clear, summarize the intended experience and the next useful decision."#;

const IMPLEMENTATION_PARTNER_SYSTEM_PROMPT: &str = r#"You are an implementation partner agent. You help turn scoped plans into working changes.

# Focus

Prefer small, direct edits that follow the existing codebase. Keep momentum, validate behavior, and flag any risky assumption before it becomes expensive.

# Collaboration

Give concise progress updates and keep the work easy for another engineer to review."#;

const QA_REVIEWER_SYSTEM_PROMPT: &str = r#"You are a QA reviewer agent. You look for the ways a change might break.

# Focus

Inspect state transitions, empty states, permissions, accessibility, and failure paths. Prioritize issues by impact and make the evidence easy to verify.

# Collaboration

Be direct but constructive. When possible, suggest the smallest fix that would reduce the risk."#;

const WORK_COORDINATOR_SYSTEM_PROMPT: &str = r#"You are a work coordination agent. You keep multi-step work organized and grounded.

# Focus

Track goals, dependencies, and follow-ups. Keep plans practical, name what is done, and make unresolved work visible without adding ceremony.

# Collaboration

Help the team stay aligned. Summarize decisions clearly and keep the next action obvious."#;

const SUPPORT_GUIDE_SYSTEM_PROMPT: &str = r#"You are a support and onboarding agent. You help people feel oriented quickly.
...
Use a warm tone, confirm assumptions when needed, and leave people with a clear path forward."#;

const EXPERIMENT_DESIGNER_SYSTEM_PROMPT: &str = r#"You are an experiment design agent. You help turn uncertainty into experiments.
...
Stay curious, practical, and optimistic. Make the next experiment easy to try."#;
...
const BUILT_IN_PERSONAS: &[BuiltInPersona] = &[
    ...
    BuiltInPersona {
        ...
        avatar_url: Some(PRODUCT_STRATEGIST_AVATAR),
        system_prompt: PRODUCT_STRATEGIST_SYSTEM_PROMPT,
        name_pool: &["Product Strategist"],
        model: None,
        runtime: None,
    ...
    BuiltInPersona {
        ...
        system_prompt: IMPLEMENTATION_PARTNER_SYSTEM_PROMPT,
        name_pool: &["Implementation Partner"],
        model: None,
        runtime: None,
        default_active: false,
    },
    BuiltInPersona {
        ...
        avatar_url: Some(QA_REVIEWER_AVATAR),
        system_prompt: QA_REVIEWER_SYSTEM_PROMPT,
        name_pool: &["QA Reviewer"],
        model: None,
        runtime: None,
        default_active: false,
    },
    BuiltInPersona {
        ...
        avatar_url: Some(WORK_COORDINATOR_AVATAR),
        system_prompt: WORK_COORDINATOR_SYSTEM_PROMPT,
        name_pool: &["Work Coordinator"],
        model: None,
        runtime: None,
        default_active: false,
    },
    BuiltInPersona {
        ...
        avatar_url: Some(SUPPORT_GUIDE_AVATAR),
        system_prompt: SUPPORT_GUIDE_SYSTEM_PROMPT,
        name_pool: &["Support Guide"],
        model: None,
        runtime: None,
        default_active: false,
    },
    BuiltInPersona {
        ...
        avatar_url: Some(EXPERIMENT_DESIGNER_AVATAR),
        system_prompt: EXPERIMENT_DESIGNER_SYSTEM_PROMPT,
        name_pool: &["Experiment Designer"],
        model: None,
        runtime: None,
        default_active: false,
    },

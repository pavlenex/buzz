...
type UseAnchoredScrollOptions = {
  ...
  /** Bumps after a retained-window transaction so an off-window target can retry. */
...
export function useAnchoredScroll({
  ...
  // ---------------------------------------------------------------------------
  // biome-ignore lint/correctness/useExhaustiveDependencies: `messages` and `renderVersion` are intentional triggers, not reads — the effect reads the DOM, and retries whenever data or the retained row window commits.
  React.useEffect(() => {
    if (!targetMessageId) {
      handledTargetIdRef.current = null;
      return;

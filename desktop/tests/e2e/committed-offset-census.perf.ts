...
type Frame = {
  ...
  scrollTop: number;
  rowId: string | null;
  // LANE C committed-offset candidate signals — each is a hypothesis for
  // "a JS-readable value that tracks the compositor-committed offset when
  // scrollTop read is stale/frozen". Recorded every frame so we can ask, on
  // the frozen-scrollTop frames (dScroll==0 & rowMove reversed), whether the
  // candidate moved when scrollTop did not.
  vvOffsetTop: number | null; // visualViewport.offsetTop
  vvPageTop: number | null; // visualViewport.pageTop
  stTime: number | null; // ScrollTimeline.currentTime (progress), if supported
  fixedRefTop: number; // getBoundingClientRect().top of a position:fixed ref
  scrollerRectTop: number; // scroller getBoundingClientRect().top
};
...
test("PORTABLE upscroll classify: reflow-driven vs tracking-failure reversals", async ({
  ...
  await timeline.evaluate((element, margin: number) => {
    ...
    const w = window as unknown as {
      ...
      __CTIMELINE__?: { currentTime: { value: number } | number | null };
      __FIXEDREF__?: HTMLElement;
    };
    type Frame = {
      t: number;
      top: number | null;
    ...
    // Inject a position:fixed reference. Its viewport rect stays constant
    // regardless of scroll, so (scrollerRectTop - fixedRefTop) does NOT change
    // with scroll — but a mounted ROW's rect DOES track the compositor visual
    // position (that is the frozen-frame tell). We record fixedRefTop only to
    // prove the fixed ref is genuinely scroll-invariant (control), and the
    // scroller rect to confirm the scroller box itself does not move.
    if (!w.__FIXEDREF__) {
    ...
    // ScrollTimeline candidate: compositor-driven progress by spec design.
    if (!w.__CTIMELINE__ && "ScrollTimeline" in window) {
      try {
        w.__CTIMELINE__ = new (
          window as unknown as {
            ScrollTimeline: new (o: {
              source: Element;
              axis: string;
            }) => {
              currentTime: { value: number } | number | null;
            };
          }
        ).ScrollTimeline({ source: el, axis: "block" });
      } catch {
        w.__CTIMELINE__ = { currentTime: null };
      }
    }
    ...
    const readST = (): number | null => {
      const ct = w.__CTIMELINE__?.currentTime;
      if (ct == null) return null;
      return typeof ct === "number" ? ct : ct.value;
    };
    const tick = (t: number) => {
      ...
      w.__PCLASS__.frames.push({
        ...
        stTime: readST(),
        fixedRefTop: w.__FIXEDREF__!.getBoundingClientRect().top,
        scrollerRectTop: box.top,
      });
  ...
  // ============================================================
  // LANE C COMMITTED-OFFSET CENSUS
  // The frozen-scrollTop reversal is the target: a frame pair where the anchor
  // row reversed against the upscroll (rowMove <= -REVERSAL_PX) while scrollTop
  // reported ~zero delta (|dScroll| < 1) and no reflow moved it (|aboveΔ| < 1).
  // On such a frame the compositor advanced the visual layer but the main-thread
  // scrollTop read is stale. The arm's whole premise: is there a JS-readable
  // signal that DID move on that frame (i.e. tracked the committed offset)?
  //
  // For each frozen reversal frame we print the per-candidate delta. A candidate
  // that consistently shows a non-zero delta ~matching rowMove on frozen frames
  // is a viable committed-offset source. A candidate that is also frozen (~0
  // delta) is useless — it shares scrollTop's staleness.
  type FrozenRow = {
    i: number;
    rowMove: number;
    dScroll: number;
    aboveDelta: number;
    dVvOffsetTop: number | null;
    dVvPageTop: number | null;
    dStTime: number | null;
    dFixedRefTop: number;
    dScrollerRectTop: number;
  };
  const d0 = (x: number | null, y: number | null): number | null =>
  ...
  for (let i = 1; i < frames.length; i += 1) {
    ...
    // Frozen-scrollTop tracking-failure reversal: scrollTop still, no reflow.
    if (Math.abs(dScroll) >= 1 || Math.abs(aboveDelta) > ABOVE_EPS) continue;
    const d = d0;
    frozen.push({
      i,
      rowMove,
      dScroll,
      aboveDelta,
      dVvOffsetTop: d(b.vvOffsetTop, a.vvOffsetTop),
      dVvPageTop: d(b.vvPageTop, a.vvPageTop),
      dStTime: d(b.stTime, a.stTime),
      dFixedRefTop: b.fixedRefTop - a.fixedRefTop,
      dScrollerRectTop: b.scrollerRectTop - a.scrollerRectTop,
    });
  }

  const nz = (vals: (number | null)[]): number =>
    vals.filter((v) => v != null && Math.abs(v) > 0.5).length;
  const stSupported = frames.some((f) => f.stTime != null);
  const vvSupported = frames.some((f) => f.vvOffsetTop != null);

  // LIVENESS CONTROL: do the candidate signals EVER move across the whole run?
  // If a signal is 0 on frozen frames AND 0 everywhere, it's a dead API (never
  // tracked scroll at all) — a different, weaker finding than "tracks scroll
  // but is frozen exactly when scrollTop is". We need to know which. Count
  // frame-pairs (any, not just reversals) where each signal showed a delta.
  let liveVvOff = 0;
  let liveVvPage = 0;
  let liveSt = 0;
  let liveScrollTop = 0;
  for (let i = 1; i < frames.length; i += 1) {
    const a = frames[i - 1];
    const b = frames[i];
    if (Math.abs(b.scrollTop - a.scrollTop) > 0.5) liveScrollTop += 1;
    const dvo = d0(b.vvOffsetTop, a.vvOffsetTop);
    const dvp = d0(b.vvPageTop, a.vvPageTop);
    const dst = d0(b.stTime, a.stTime);
    if (dvo != null && Math.abs(dvo) > 0.5) liveVvOff += 1;
    if (dvp != null && Math.abs(dvp) > 0.5) liveVvPage += 1;
    if (dst != null && Math.abs(dst) > 0.0001) liveSt += 1;
  }
  ...
  console.log(`all reversals:          ${allReversals}`);
  console.log(
    `FROZEN reversals:       ${frozen.length} (dScroll~0, no reflow)`,
  );
  console.log("--- candidate support ---");
  console.log(`  ScrollTimeline present: ${stSupported}`);
  console.log(`  visualViewport present: ${vvSupported}`);
  console.log(
    "--- LIVENESS (frame-pairs with a delta across the WHOLE run) ---",
  );
  console.log(
    `  scrollTop moved:        ${liveScrollTop}/${scored + reanchors}`,
  );
  console.log(`  visualViewport.offsetTop moved: ${liveVvOff}`);
  console.log(`  visualViewport.pageTop moved:   ${liveVvPage}`);
  console.log(`  ScrollTimeline.currentTime moved: ${liveSt}`);
  console.log(
    "--- on FROZEN frames, how many showed a NON-ZERO delta (>0.5px/unit)? ---",
  );
  console.log(
    `  rowMove (control — always nonzero): ${nz(frozen.map((f) => f.rowMove))}/${frozen.length}`,
  );
  console.log(
    `  visualViewport.offsetTop:           ${nz(frozen.map((f) => f.dVvOffsetTop))}/${frozen.length}`,
  );
  console.log(
    `  visualViewport.pageTop:             ${nz(frozen.map((f) => f.dVvPageTop))}/${frozen.length}`,
  );
  console.log(
    `  ScrollTimeline.currentTime:         ${nz(frozen.map((f) => f.dStTime))}/${frozen.length}`,
  );
  console.log(
    `  fixedRef rect.top (control ~0):     ${nz(frozen.map((f) => f.dFixedRefTop))}/${frozen.length}`,
  );
  console.log(
  ...
  for (const f of frozen.slice(0, 12)) {
    console.log(
      `  f${f.i} rowMove=${f.rowMove.toFixed(1)} dScroll=${f.dScroll.toFixed(1)} aboveΔ=${f.aboveDelta.toFixed(1)} | dVVoff=${f.dVvOffsetTop?.toFixed(2) ?? "n/a"} dVVpage=${f.dVvPageTop?.toFixed(2) ?? "n/a"} dST=${f.dStTime?.toFixed(4) ?? "n/a"} dFixedRef=${f.dFixedRefTop.toFixed(2)} dScrollerRect=${f.dScrollerRectTop.toFixed(2)}`,
    );
  }
  console.log("======================================\n");
  /* eslint-enable no-console */

  // Only hard assertion: we produced a scored upscroll to census over.

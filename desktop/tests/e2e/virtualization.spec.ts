...
test.describe("list virtualization", () => {
  ...
  test("08 — cascading older pages never snap the viewport toward newest", async ({
    ...
    // Load three consecutive server pages in one mounted virtualizer. This is
    // the production shape that exposed the intermittent end-cache snap: each
    // restored position can immediately approach the next history boundary.
    for (let pageIndex = 0; pageIndex < 3; pageIndex += 1) {
      ...
      expect(Math.abs(after.top - before.top)).toBeLessThan(120);
